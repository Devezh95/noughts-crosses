...
export function GameInfo({
  ...
  return (
    <div
      className={clsx(
        className,
        "bg-white rounded-2xl shadow-md px-8 py-4 flex justify-between grid grid-cols-2 gap-3",
      )}
    >
      {players.slice(0, playersCount).map((player, index) => (
        <PlayerInfo
          ...
          isTimmerRunnig={currentMove === player.symbol && !isWinner}
        />
      ))}
    </div>
  );
...
function PlayerInfo({ playerInfo, isRight, isTimmerRunnig, onTimeOver }) {
